...
class SigLIP(ImageEmbeddingModel):
    ...
    def __init__(self, device: torch.device) -> None:
        ...
        self._model = AutoModel.from_pretrained("google/siglip-base-patch16-224").to(device=self._device)
    ...
    def __call__(self, x: Image.Image) -> torch.Tensor:
        with torch.no_grad():
            return self._model.get_image_features(**self._preprocess(image=x)).to(device=self._device)
