...
def main():
    ...
    anytraverse = create_anytraverse_hoc_context(
        init_prompts=[("dirt path", 1.0), ("grass", 1.0), ("bush", -1.0)],
    ...
    with depthai.Device(pipeline=depthai_pipeline) as depthai_device:
        ...
        while True:
            ...
            # )
            # path_img = cv2.resize(
            #     path_img, (image.shape[0], image.shape[0]),
            #     interpolation=cv2.INTER_NEAREST
            # )
            # disp_image = np.concatenate((image, path_img), axis=1)
            # disp_image = imutils.resize(disp_image, width=1280)
            # cv2.imshow("Image and Path", disp_image)
            #
            # LOOKAHEAD = 0.7
            # if len(path) > int(1.0 / costmap._resolution):
            #     # Select initial motion direction
            #     w0, w1 = path[[0, int(LOOKAHEAD / costmap._resolution)]]
